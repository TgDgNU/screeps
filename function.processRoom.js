...
var processRoom = {
    findEnergy: function (roomName) {
        ...
        for (let id in dropppedEnergy) {
            roomEnergyArray.push(["droppedEnergy", dropppedEnergy[id].amount, dropppedEnergy[id].pos])
        }
        ...
        for (let id in container) {
            roomEnergyArray.push(["container", container[id].store.energy, container[id].pos])
        }
        ...
        for (let id in storage) {
            roomEnergyArray.push(["storage", storage[id].store.energy, storage[id].pos])
        }
